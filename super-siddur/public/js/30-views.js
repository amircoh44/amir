...
async function renderTehillimRead(stage,n){
  ...
  if(!verses){
    if(navigator.onLine===false){
      ...
      try{const r=await fetch(`https://www.sefaria.org/api/v3/texts/Psalms.${n}?return_format=text_only`);if(r.ok){const d=await r.json();verses=((d.versions&&d.versions[0]&&d.versions[0].text)||[]).map(cleanText);if(verses&&verses.length){try{const c=JSON.parse(localStorage.getItem("teh_cache")||"{}");c[n]=verses;localStorage.setItem("teh_cache",JSON.stringify(c));}catch(e){}}}}catch(e){verses=null;}
    }
  }
  if(verses&&verses.length){wrap.innerHTML="";verses.forEach((v,i)=>{const b=el("div","block");const he=el("div","he-text");he.innerHTML=`<span style="color:var(--muted);font-size:.7em;margin-left:.5em">${gematria(i+1)}.</span> ${esc(cleanText(v))}`;b.appendChild(he);wrap.appendChild(b);});}
