...
function startOnboard(){obStep=0;obData={hebName:state.userHebName||"",engName:state.userEngName||"",birthday:state.birthday,nusach:state.nusach,loc:state.loc,israelMode:state.israelMode};$("#cover").classList.add("gone");$("#onboard").classList.add("show");$("#app").style.visibility="hidden";renderOb();}
function renderOb(){
  ...
  const nx=$("#obNext");if(nx)nx.onclick=()=>{if(obStep===3){state.onboarded=true;Object.assign(state,{userHebName:obData.hebName,userEngName:obData.engName,birthday:obData.birthday,nusach:obData.nusach,loc:obData.loc,israelMode:obData.israelMode});saveState();$("#onboard").classList.remove("show");openApp();}else{obStep++;renderOb();}};
...
function renderZmanim(stage){
  ...
  function paintClock(){try{const t=new Intl.DateTimeFormat("en-US",{timeZone:tz,hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false}).format(new Date());clock.innerHTML=`<span style="color:var(--muted);font-family:var(--sans);font-size:.66rem;letter-spacing:.18em;text-transform:uppercase">Local time</span><span style="color:var(--accent);font-weight:500;font-size:1rem">${t}</span>`;}catch(e){}}
  ...
  Z.forEach(([k,en,he])=>{const isNext=next&&next.k===k;const row=el("div");row.style.cssText=`display:flex;justify-content:space-between;align-items:center;padding:.95rem 0;border-bottom:1px solid var(--line);${isNext?'background:color-mix(in srgb,var(--accent) 8%,transparent);border-radius:.6rem;padding:.95rem 1rem;border-bottom:0;border:1px solid color-mix(in srgb,var(--accent) 25%,transparent);margin:.3rem 0':''}`;row.innerHTML=`<div><div style="font-family:var(--display);font-weight:500;font-size:1rem;color:var(--ink)">${esc(en)}${isNext?' <span style="font-size:.62rem;letter-spacing:.18em;text-transform:uppercase;color:var(--accent);font-weight:600;margin-left:.4rem">NEXT</span>':''}</div><div style="color:var(--muted);font-family:var(--hebrew);direction:rtl;font-size:.95rem;margin-top:.18rem">${esc(he)}</div></div><div style="font-family:var(--mono);font-size:1.1rem;color:var(--accent);font-weight:500;font-variant-numeric:tabular-nums">${hmFmt(z[k])}</div>`;stage.appendChild(row);});
